...
double update_avg(avg_data_t *avgdata, int index, double newval)
{
  ...
  if (avgdata->effdepth < avgdata->avgdepth) // depth not filled yet
  {
    ...
    avgdata->avg[index] = (avgdata->effdepth * avgdata->avg[index] + newval) / (avgdata->effdepth + 1); // accumulate
average
    //avgdata->effdepth++;
  }
  else
...
double update_avg_sumextreme(avg_data_t *avgdata, int N, float *psd, int max0, int minbin, int maxbin, int *peakbin) {
    ...
    for (index = 0; index < N; index++) {
        if ((index < minbin) || (index >= maxbin))
            ...
            /* printf("avgdata->cum[%i]=%le avgdata->avg[%i]=%le min=%le max-min=%le\n", index, avgdata->cum[index],
             * index, avgdata->avg[index], min, max-min); */
        }
    }
...
double update_avg_sumavg(avg_data_t *avgdata, int N, float *psd, int max0, int minbin, int maxbin, int *peakbin,
                         double *variance) {
    double max = psd[minbin];
    double avgspec = 0.0;
    double sum_avg;
    ...
    *variance = 0.0;                                            // reset variance accumulator
    ...
    for (index = 0; index < N; index++) {
        if ((index < minbin) || (index >= maxbin))
            ...
            sum_avg =
                avgdata->cum[index] - avgspec;  // sum minus average over spectrum gives (anticipated) signal delta
            if (sum_avg > 0) {
                if (max0)
                    avgdata->avg[index] = (avgdata->cum[index] - avgspec) /
                        (max - avgspec);  // S+N/N clipped to maximum not scaled to time integration
                else
                    avgdata->avg[index] = avgdata->cum[index] / avgspec;
             ...
             * avgdata->cum[index], index, avgdata->avg[index], avgspec, max-avgspec); */
        }
    }
