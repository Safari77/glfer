...
static GtkWidget *scrolled_window, *stop_start_bu, *cpu_la, *pwr_la, *peak_la,
    *avgmax_frame = NULL, *avgmax_la, *avgvar_frame = NULL, *avgvar_la, *avgfill_frame = NULL, *avgfill_la,
    *peakfreq_frame = NULL, *peakfreq_la;
...
static GtkItemFactoryEntry menu_items[] = {
    ...
    {   "/Source/_Open file", "<control>O",   (GtkItemFactoryCallback)open_file_dialog,           0,           NULL},
    {  "/Source/_Sound card", "<control>S", (GtkItemFactoryCallback)select_soundcard_source,      0,           NULL},
    {         "/Source/sep1",         NULL,                                        NULL,           0,  "<Separator>"},
    {         "/Source/Quit", "<control>Q",              (GtkItemFactoryCallback)gtk_main_quit,   0,           NULL},
    {               "/_Mode",         NULL,                                        NULL,           0,     "<Branch>"},
    {           "/_Mode/FFT",         NULL,             (GtkItemFactoryCallback)select_mode,    MODE_FFT,  "<RadioItem>"},
    {           "/_Mode/MTM",         NULL,             (GtkItemFactoryCallback)select_mode,    MODE_MTM,    "/Mode/FFT"},
    {        "/_Mode/HPARMA",         NULL,             (GtkItemFactoryCallback)select_mode, MODE_HPARMA,    "/Mode/FFT"},
    {           "/_Mode/LMP",         NULL,             (GtkItemFactoryCallback)select_mode,    MODE_LMP,    "/Mode/FFT"},
    {            "/Settings",         NULL,                                        NULL,           0,     "<Branch>"},
    {   "/Settings/Spectrum",         NULL,    (GtkItemFactoryCallback)spec_settings_dialog,      0,           NULL},
    {       "/Settings/QRSS",         NULL,    (GtkItemFactoryCallback)qrss_settings_dialog,      0,           NULL},
    {       "/Settings/Port",         NULL,    (GtkItemFactoryCallback)port_settings_dialog,      0,           NULL},
    {      "/Settings/Audio",         NULL,   (GtkItemFactoryCallback)audio_settings_dialog,      0,           NULL},
    ...
    {         "/_Help/About",         NULL,              (GtkItemFactoryCallback)show_about,      0,           NULL},
    {       "/_Help/License",         NULL,            (GtkItemFactoryCallback)show_license,      0,           NULL},
};
...
static gint update_pwr(gpointer data) {
    ...
    g_string_sprintf(buf_1, "   S: %5.3lf  N: %5.3lf S/N: %5.3lf  ", 10 * log10(sig_mean_pwr),
                     10 * log10(floor_mean_pwr), 10 * log(sig_mean_pwr) - 10 * log(floor_mean_pwr));
    gtk_label_set_text(GTK_LABEL(label), buf_1->str);
...
static gint update_peak(gpointer data) {
    ...
    g_string_sprintf(buf_1, " Power: %5.3le at bin %5.1i ", peak_pwr, peak_bin);
    gtk_label_set_text(GTK_LABEL(label), buf_1->str);
...
static gboolean cols_da_expose_event(GtkWidget *widget, GdkEventExpose *event, gpointer user_data) {
    ...
    gdk_draw_drawable(widget->window, widget->style->fg_gc[GTK_WIDGET_STATE(widget)], spec_pixmap, event->area.x,
                      pixmap_ofs + event->area.y, event->area.x, event->area.y, event->area.width, event->area.height);
...
static int spec_da_configure_event(GtkWidget *widget, GdkEventConfigure *event, gpointer user_data) {
    ...
    /* clear the new pixmap */
    gdk_draw_rectangle(spec_pixmap, widget->style->fg_gc[GTK_WIDGET_STATE(drawing_area)], TRUE, 0, 0, SPEC_DA_WIDTH,
                       pixmap_height);
...
static gboolean drawing_area_expose_event(GtkWidget *widget, GdkEventExpose *event, gpointer user_data) {
    /* copy the relevant portion of the pixmap onto the screen */
    gdk_draw_drawable(widget->window, widget->style->fg_gc[GTK_WIDGET_STATE(widget)], pixmap, event->area.x,
                      pixmap_ofs + event->area.y, event->area.x, event->area.y, event->area.width, event->area.height);
...
static int drawing_area_configure_event(GtkWidget *widget, GdkEventConfigure *event, gpointer user_data) {
    ...
    D(printf("configure event: widget = %p, all_height = %i, all_width = %i\n", widget, widget->allocation.height,
             widget->allocation.width));                                                              /* for debug */
    ...
    /* clear the new pixmap */
    gdk_draw_rectangle(new_pixmap, widget->style->fg_gc[GTK_WIDGET_STATE(drawing_area)], TRUE, 0, 0, pixmap_width,
                       pixmap_height);

    if (pixmap) {
        gdk_draw_drawable(new_pixmap, widget->style->fg_gc[GTK_WIDGET_STATE(widget)], pixmap, 0, 0, 0, 0,
                          (new_l > l) ? l : new_l, (new_n_zoom > n_zoom) ? n_zoom * n : new_n_zoom * n);
...
static gboolean drawing_area_motion_event(GtkWidget *widget, GdkEventMotion *event, gpointer data) {
    ...
    g_string_sprintf(buf_1, "%.3f Hz %.3f/%.3fs %3.2i dB", da_to_act_y(event->y), da_to_act_x(event->x),
                     da_to_act_x_rel(event->x), levbuf[(int)(event->x * pixmap_height + event->y)]);
    gtk_label_set_text(GTK_LABEL(label), buf_1->str);
...
static void selection_event(GtkWidget *widget, GdkEventMotion *event, gpointer data) {
    ...
    gdk_draw_drawable(widget->window, widget->style->fg_gc[GTK_WIDGET_STATE(widget)], pixmap, x_left, y_top, x_left,
                      y_top, x_right - x_left, 1);
    /* left vertical line of rectangle */
    gdk_draw_drawable(widget->window, widget->style->fg_gc[GTK_WIDGET_STATE(widget)], pixmap, x_left, y_top, x_left,
                      y_top, 1, y_bottom - y_top);
    /* bottom horizontal line of rectangle */
    gdk_draw_drawable(widget->window, widget->style->fg_gc[GTK_WIDGET_STATE(widget)], pixmap, x_left, y_bottom, x_left,
                      y_bottom, x_right - x_left + 1, 1);
    /* right vertical line of rectangle */
    gdk_draw_drawable(widget->window, widget->style->fg_gc[GTK_WIDGET_STATE(widget)], pixmap, x_right, y_top, x_right,
                      y_top, 1, y_bottom - y_top);
...
static void release_event(GtkWidget *widget, GdkEventButton *event, gpointer data) {
    ...
    D(printf("x_left = %f, y_top = %f, x_right = %f, y_bottom = %f\n", x_left, y_top, x_right,
             y_bottom)); /* for debug */

    if (event->button == 1) {
        pixbuf = gdk_pixbuf_get_from_drawable(pixbuf, pixmap, gdk_rgb_get_colormap(), x_left, y_top, 0, 0, sel_width,
                                              sel_height);
...
GtkWidget *main_window_init(int nr, float min, float max) {
    ...
    if (window == NULL) {
        ...
        */
        ...
        g_signal_connect(G_OBJECT(drawing_area), "button_press_event", G_CALLBACK(drawing_area_button_press_event),
                         NULL);
    ...
    gtk_widget_show(cpu_la);
        ...
        //    gtk_drawing_area_size(GTK_DRAWING_AREA(drawing_area), drawing_area->allocation.x,
...
void main_window_draw(float *psdbuf) {
    ...
    gdk_draw_rectangle(spec_pixmap, spec_da->style->fg_gc[GTK_WIDGET_STATE(drawing_area)], TRUE, 0, 0, SPEC_DA_WIDTH,
                       pixmap_height);
#if 0
  ...
  for (i = 0; i < n; i++)
    ...
    if (opt.autoscale) {  // EG only for (original) autoscale mode
        if (glfer.first_buffer == TRUE) {
            /* when settings are changed need to estimate the PSD taking into account that the "old" samples are all
             * zero and only the "overlapped part" is actually filled with real data */
        ...
        display_min_lvl = (display_max_lvl > display_min_lvl ? display_min_lvl
                                                             : display_max_lvl / 10.0);  // EG prevent stupid entries
    ...
    // display_max_lvl, display_min_lvl); fprintf(stderr, "display_max = %e\tdisplay_min = %e\n", display_max,
    // display_min);
    ...
    if ((opt.scale_type == SCALE_LIN_MAX0) ||
        (opt.scale_type == SCALE_LOG_MAX0))  // EG set the max to 0dB indicator for sumavg and sumextreme
        max0 = 1;
    else
        max0 = 0;
    ...
    x_cur = x;
    gdk_draw_rgb_image(pixmap, drawing_area->style->fg_gc[GTK_STATE_NORMAL], x, 0, 1, n_zoom * n, GDK_RGB_DITHER_NONE,
                       rgbbuf, 3);
...
void save_full_pixmap(GtkWidget *widget, gpointer data) {
    pixbuf =
        gdk_pixbuf_get_from_drawable(NULL, pixmap, gdk_rgb_get_colormap(), 0, 0, 0, 0, pixmap_width, pixmap_height);
