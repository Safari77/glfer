...
void save_spect_dialog(GtkWidget *widget, gpointer data) {
    ...
    dialog = gtk_file_chooser_dialog_new("Save File", NULL, GTK_FILE_CHOOSER_ACTION_SAVE, GTK_STOCK_CANCEL,
...
static void spectrum_file_save(gchar *filename) {
    ...
    else if (g_str_has_suffix(filename, ".png") || g_str_has_suffix(filename, ".PNG"))
        save_ok = gdk_pixbuf_save(pixbuf, filename, "png", &error, NULL);
    else if (g_str_has_suffix(filename, ".bmp") || g_str_has_suffix(filename, ".BMP"))
