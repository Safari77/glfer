...
extern size_t num_fft_windows;

static GtkWidget *spec_window = NULL, *qrss_window = NULL, *tx_test_window = NULL, *port_window = NULL,
                 *audio_window = NULL;
static GtkWidget *sn_bu, *be_keyup_bu, *be_keydown_bu, *qrss_bu, *dfcw_bu, *serial_bu, *parallel_bu, *hendrix_bu,
    *autoscale_bu;
static GtkWidget *offset_en, *data_block_size_en, *data_blocks_overlap_en, *display_update_en, *limiter_a_en, *mtm_w_en,
    *mtm_k_en, *hparma_t_en, *hparma_p_e_en, *lmp_av_en, *sample_rate_en, *dot_time_en, *control_port_en, *wpm_en,
    *lin_scale_bu, *log_scale_bu, *ptt_delay_en, *sidetone_fr_en, *dfcw_dot_fr_en, *dfcw_dash_fr_en, *be_pause_en,
    *level_en, *lin_scale_bu, *lin_scale_max0_bu, *log_scale_bu, *log_scale_max0_bu, *avg_sumavg_bu, *avg_plain_bu,
    *avg_sumextreme_bu, *noavg_bu, *max_level_en, *min_level_en, *avgsamples_en, *minavgband_en, *maxavgband_en,
    *audio_device_en, *avgtime_la, *avgintdb_la, *bin_size_la;
;
...
void tx_test_dialog(GtkWidget *widget, gpointer data) {
    ...
    g_signal_connect_swapped(G_OBJECT(temp_bu), "clicked", G_CALLBACK(close_tx_test_dialog),
                             GTK_OBJECT(tx_test_window));
...
static void spec_widgets_to_prefs(GtkWidget *widget, gpointer data) {
    ...
    if (tmp_int != opt.avgsamples)  // FIXME: ahould call alloc_avg also at first average usage (first call)
    {
        ...
        delete_avg(&avgdata);
        alloc_avg(&avgdata, opt.data_block_size, opt.avgsamples);
        tmp_str = g_strdup_printf("Integration gain %.1f dB",
                                  10.0 * log10(((float)opt.avgsamples) * (1.0 - opt.data_blocks_overlap)));
        gtk_label_set_text(GTK_LABEL(avgintdb_la), tmp_str);
        g_free(tmp_str);
        glfer.avgfill = 0;  // reset ratio of average buffer full
        glfer.avgtime = (float)opt.avgsamples * (float)opt.data_block_size * (1.0 / (float)opt.sample_rate) *
            (1.0 - opt.data_blocks_overlap);
    ...
    if (tmp_int != opt.sample_rate)  // EG changed?
    {
        ...
        tmp_str = g_strdup_printf("Resulting bin size %3.3f Hz",
                                  (float)opt.sample_rate / (float)opt.data_block_size);  // update bin size label
...
void spec_settings_dialog(GtkWidget *widget, gpointer data) {
    ...
    gtk_tooltips_set_tip(glfer.tt, limiter_a_en,
                         "EXPERIMENTAL: RA9MB nonlinear processing A parameter. 0.0 is conventional periodogram, "
                         "otherwise 0.001 is a good starting point",
                         NULL);
    ...
    gtk_tooltips_set_tip(glfer.tt, hparma_t_en, "Number of equations used to solve the extended Yule-Walker equation",
                         NULL);
    ...
    gtk_tooltips_set_tip(glfer.tt, log_scale_max0_bu,
                         "Set logarithmic scale with maximum set at 0dB (sumavg and sumextreme only)", NULL);
    ...
    gtk_tooltips_set_tip(glfer.tt, lin_scale_max0_bu,
                         "Set linear scale with maximum set at 0dB (sumavg and sumextreme only)", NULL);
...
static void display_update_en_changed(GtkWidget *widget, gpointer *data) {
    ...
    if ((tmp_overlap < 0) || (tmp_overlap >= 1.0)) {
        ...
        gtk_signal_handler_block_by_func(GTK_OBJECT(data_blocks_overlap_en), G_CALLBACK(data_blocks_overlap_en_changed),
                                         data);
        gtk_entry_set_text(GTK_ENTRY(data_blocks_overlap_en), tmp_str);
        gtk_signal_handler_unblock_by_func(GTK_OBJECT(data_blocks_overlap_en),
                                           G_CALLBACK(data_blocks_overlap_en_changed), data);
        g_free(tmp_str);
    }
}
...
static void port_widgets_to_prefs(GtkWidget *widget, gpointer data) {
    ...
    if ((glfer.init_done == FALSE) || strcmp(opt.ctrl_device, selected_device)) {
        ...
        label = gtk_label_new(
            "Selected control port may become unstable during initialisation!\nPlease make sure that the TX is off.");
...
void port_settings_dialog(GtkWidget *widget, gpointer data) {
    ...
    gtk_tooltips_set_tip(glfer.tt, control_port_en, "Control port name (e.g. ttyS0 for serial, lp0 for parallel)",
                         NULL);
...
void audio_settings_dialog(GtkWidget *widget, gpointer data) {
    ...
    g_signal_connect_swapped(G_OBJECT(temp_bu), "clicked", G_CALLBACK(audio_widgets_to_prefs),
                             GTK_OBJECT(audio_window));
