...
static void audio_available(gpointer data, gint source, GdkInputCondition condition) {
    ...
    for (i = 0; i < n; i++) {
        ...
        case MODE_FFT:
            ...
            if (glfer.scope_window)
                scope_window_draw(fft_gram_par);
            break;
        case MODE_MTM:
            mtm_do(audio_buf + i * n_eff, psdbuf, NULL, &mtm_gram_par);
            if (glfer.scope_window)
                scope_window_draw(mtm_gram_par.fft);
            break;
        case MODE_HPARMA:
            hparma_do(audio_buf + i * n_eff, psdbuf, NULL, &hparma_gram_par);
            if (glfer.scope_window)
                scope_window_draw(hparma_gram_par.fft);
            break;
        case MODE_LMP:
            lmp_do(audio_buf + i * n_eff, psdbuf, NULL, &lmp_gram_par);
            if (glfer.scope_window)
                scope_window_draw(lmp_gram_par.fft);
            break;
        default:
...
void change_params(int mode_sel) {
    ...
    case MODE_MTM:
        ...
        break;
    case MODE_HPARMA:
        if ((hparma_gram_par.fft.n != opt.data_block_size) ||
