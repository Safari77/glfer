...
static gboolean scope_da_expose_event(GtkWidget *widget, GdkEventExpose *event, gpointer user_data) {
    ...
    gdk_draw_drawable(widget->window, widget->style->fg_gc[GTK_WIDGET_STATE(widget)], scope_pixmap, event->area.x,
...
static int scope_da_configure_event(GtkWidget *widget, GdkEventConfigure *event, gpointer user_data) {
    ...
    gdk_draw_rectangle(scope_pixmap, widget->style->fg_gc[GTK_WIDGET_STATE(scope_da)], TRUE, 0, 0, SCOPE_DA_WIDTH,
                       pixmap_height);
...
void scope_window_init(GtkWidget *widget, gpointer data) {
    ...
    if (window == NULL) {
        ...
        gtk_widget_set_events(scope_da,
...
void scope_window_draw(fft_params_t params) {
    ...
    if (autoscale) {
        ...
        if (datamax < 1e-3)
            datamax = 1e-3; /* avoid division by zero */
    ...
    gdk_draw_rectangle(scope_pixmap, scope_da->style->fg_gc[GTK_WIDGET_STATE(scope_da)], TRUE, 0, 0, SCOPE_DA_WIDTH,
                       pixmap_height);
